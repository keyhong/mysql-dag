"""
DAG file that takes the operator's container and sequentially executes task-specific tasks
"""
...
from settings import HOST, PORT, USER, PASSWORD, DB, CHARSET, CHECK_COUNT, CHECK_CYCLE
...
def record_time_status(pid: int):
    ...
    while cnt <= CHECK_COUNT:
        ...
        if psutil.pid_exists(pid):
            logging.debug(f'{str(datetime.now()).split(".")} 프로세스 P1이 살아있습니다.')
            cnt += 1
            continue
        else:
            logging.debug(f'{str(datetime.now()).split(".")} 프로세스 P1이 죽었습니다.')
            return
...
class MySQLDAG:
    ...
    def run(self):
        ...
        # Operator의 실패지점부터 
        for batch_idx, operator in enumerate(self.operators[start_idx:], start=start_idx):
            
            if IS_ERROR is True:
                break
            
            # LogLoader (배치 작업의 로그 적재 및 ) 인스턴스 생성
            logger = LogLoader(operator, start_idx + batch_idx)
            ...
            try:
                ...
                ** Flow : (Main Process) -> (쿼리 수행 프로세스 실행) -> (p1의 프로세스를 5분 마다 확인하는 프로세스 실행) -> (p1  종료) -> (p2 종료) -> (Main Process)
                ...
                '''
                _, sql, job_id, task_id = operator.get_exec_info()
                ...
                self.op_success[start_idx + batch_idx] = 1
