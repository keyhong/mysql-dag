...
"""
The Main module that defines the task order of the assignment and executes it as a DAG."
...
from mysql_dag import MySQLDAG
...
if __name__ == "__main__":
    ...
    # 작업 실행정보를 담은 (job_01.json, job_02.json) 로딩. 해당 폴더의 경로는 임시적으로 명시적 정의 
    job1_json = os.path.join(os.path.dirname(os.path.dirname(__file__)), "jobs", "#1", "job_01.json") # job_1

    with open(job1_json, 'r') as f:
        job_1_info = json.load(f)

    job2_json = os.path.join(os.path.dirname(os.path.dirname(__file__)), "jobs", "#2", "job_02.json") # job_2

    with open(job2_json, 'r') as f:
        job_2_info = json.load(f)

    # 정보를 담은 operator 인스턴스 생성
    operator1 = MySQLOperator(db_conn=mysql_conn, exec_info=job_1_info[0])
    operator2 = MySQLOperator(db_conn=mysql_conn, exec_info=job_1_info[1])
    operator3 = MySQLOperator(db_conn=mysql_conn, exec_info=job_1_info[2])
    operator4 = MySQLOperator(db_conn=mysql_conn, exec_info=job_2_info[0])
    operator5 = MySQLOperator(db_conn=mysql_conn, exec_info=job_2_info[1])
    operator6 = MySQLOperator(db_conn=mysql_conn, exec_info=job_2_info[2])

    # DAG에 담기. 엄밀히 구상하자면, 관련된 TAKS끼리 구성하여 이름 분류와 순서를 지정하여 DAG를 만드는 게 좋다.
    # 하지만 해당 DAG내에 자체적으로 job_id와 task_id의 index를 비교하여 순서를 재정렬하는 기능이 있어, 사실상 아무렇게 넣어도 상관은 없다. 
    my_op_lst = [operator1, operator2, operator3, operator4, operator5, operator6]
    
    my_dag = MySQLDAG(my_op_lst)
    my_dag.run()
